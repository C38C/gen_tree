...
def dist_2d(point1, point2):
    return ((point1[0]-point2[0])**2.0 + (point1[1]-point2[1])**2.0 + (point1[2]-point2[2])**2.0)**0.5
...
def centroid_triangle(point1, point2, point3):
    return ((point1[0]+point2[0]+point3[0])/3.0,(point1[1]+point2[1]+point3[1])/3.0,(point1[2]+point2[2]+point3[2])/3.0)
...
def generateEllipsoidVertices(vertices_count, height, radius):
    ...
    while len(vertices) < vertices_count:
        ...
        if t1 < 1.0 and t2 >= 0.0:
            ...
            z = t2
            mu = np.linalg.norm([radius*height*y, radius*radius*z, radius*height*x])
...
def selectFaces(vertices, faces, area, gapPercentage, clusterDensity = 3.0):
    ...
    while total_kept < goal_to_keep and (total_time < 50000):
        ...
        choice = random.randint(0,1) # random selection of fill option: single leaf vs. cluster
        ...
        if choice == 0: # single, lonely leaf
            ...
            for cf in close_faces: # for each close face
                if (cf not in face_keep_indices) and (random.randint(0,8) > 1): # 2/7 chance of hole in cluster of leaves
                    face_keep_indices.append(cf)
                    total_kept += triangle_areas[cf]

    print "total_kept", total_kept
    print "Time elapsed"
